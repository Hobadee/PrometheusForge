...
3. Robust Overlays - allow polymorphic overlaying and section overlays (in progress - see Recent Changes for the `PendingOverrides`/`[StepTree]`-override groundwork and the new CLI-overlay `root:`-as-overrides YAML syntax; still open: deciding whether unmatched override targets should fail fast)
